...
public class companyInfo {
  ...
  public void dailyReport(
    ...
    System.out.println(
      " Current total assets: " + CURRENCY.format(currentTotal)
    );
  ...
  public double totalOrderCost(
    LinkedList<OrderDbEntry> orders,
    LinkedList<dbEntry> products
  ) {
    double value = 0;
    int cnt = 0;
    for (OrderDbEntry order : orders) {
      ...
      if ((++cnt) % 100 == 0) {
        System.out.println(
          "Summing cost of order " + cnt + "/" + orders.size()
        );
      }
    }
    return value;
